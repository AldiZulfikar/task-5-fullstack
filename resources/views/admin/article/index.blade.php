          ...
          <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                <h6 class="text-white text-capitalize ps-3">Tabel Article</h6>
              </div>
            </div>
            <div class="card-body px-0 pb-2">
            <div class="col-12 text-end pb-4">
                <a class="btn bg-gradient-primary mb-0" href="{{route('admin-tambah-article-admin')}}"><i class="material-icons text-sm">add</i>&nbsp;&nbsp;Tambah article</a>
            </div>
                ...
                <table class="table align-items-center mb-0">
                  ...
                  @foreach ($article as $item => $data)
                    <tr>
                      ...
                      <td>
                        <div class="d-flex flex-column justify-content-center">
                          <h6 class="text-center mb-0 text-sm">{{ $data->title }}</h6>
                        </div>
                      </td>
                      <td class="align-middle text-center">
                        <div>
                          <img src="{{$data->image ? asset('upload/' .$data->image) : asset('/frontend-assets/assets/img/article.png')}}" width=100 alt="" center>
                        </div>
                      </td>
                      <td>
                        <div class="d-flex flex-column justify-content-center">
                      ...
                      <td class="align-middle text-center">
                        <a href="/admin/article/{{$data->id}}" class="badge badge-sm bg-gradient-success" data-toggle="tooltip" data-original-title="Edit user">
                          Ubah
                        </a>
                        <form onsubmit="return confirm('apakah kamu yakin ingin menghapus article ini?')" action="{{route('admin-article-delete', $data->id)}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('delete')
